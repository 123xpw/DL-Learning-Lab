...
**SAM3（2025）**：在 SAM2 基础上进一步扩展，采用双编码器-解码器架构，同时处理视觉、文本、几何提示和样例图像，视觉编码器 4.5 亿参数 + 文本编码器 3 亿参数，实现真正的多模态提示分割。

**开放词汇分割成熟化**：2025 年 CLIP 驱动的开放词汇分割从研究走向工程应用——输入任意文字描述即可分割对应区域，无需重新训练。代表工作：FC-CLIP、ODISE、以及基于 SAM + 语言模型的组合方案。

**参数高效微调（PEFT）成为主流**：直接全量微调 SAM 等大模型代价高昂，2025 年 LoRA、Adapter 等 PEFT 方法被大量用于将基础分割模型适配到医学影像、遥感、工业检测等垂直领域。

**当前格局（2026）**：

| 方向 | 代表 | 状态 |
|------|------|------|
| 通用基础模型 | SAM2 / SAM3 | 主流，持续迭代 |
| 精度 benchmark | Mask2Former + 大规模预训练 | 学术竞赛首选 |
| 开放词汇 | CLIP + SAM 组合 | 工程落地加速 |
| 轻量端侧 | SegFormer-B0、MobileViT | 嵌入式/移动端 |
